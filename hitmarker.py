...
#for determining if on heroku or not
import os

class Hitmarker(commands.Cog):
    '''
    classdocs
    '''
    ...
    def __init__(self, bot):
        ...
        if(on_heroku):
        #perform heroku selenium initialization
        ...
        #perform local selenium initalization
            ...
            #initializing driver
            self.driver = webdriver.Chrome(executable_path=path_to_webdriver, options=options)
        
    def readListings(self, driver):
        '''
    ...
    def saveNewListings(self, job_listings_list):
        '''
        ...
        for job_listing in job_listings_list:
            ...
            if(job_listing['title'] != 'ERROR'):
                ...
                hash_result = self.getHash(string_to_hash)
                #debug print
    ...
    @commands.command(name='update', help='!eba update - Have the bot search hitmarker.com for any new listings, and post them to this channel.')
    async def updateListings(self, ctx):
        await ctx.send("Searching...\nPlease wait...")
        new_listings = self.saveNewListings(self.readListings(self.driver))
        print(new_listings)
    ...
    @commands.command(name='clear', help='!eba clear - Clear the bot\'s memory so it will repost old listings it\'s already seen')
...
def setup(bot):
    '''
    Adds the Hitmarker command Cog to the bot.
    '''
    bot.add_cog(Hitmarker(bot))
